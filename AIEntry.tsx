...
const AIEntry: React.FC<Props> = ({ onClose, onSubmit }) => {
  ...
  const startRecording = async () => {
    try {
      ...
      setError(isWechat ? '请检查小程序麦克风权限。' : '无法访问麦克风，请检查浏览器权限。');
    }
  };
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        setError('未识别到账单明细，请确认内容是否清晰。');
      }
    } catch (e) {
      setError('AI 分析请求失败，请检查网络连接。');
    } finally {
      setIsParsing(false);
    }
  };
        ...
        <div className="p-6 space-y-4">
          <div className="flex gap-2">
            ...
            {audioBase64 && (
              <div className="relative h-20 flex items-center gap-2 bg-indigo-50 px-4 rounded-xl border border-indigo-100">
                ...
                <span className="text-xs font-medium text-indigo-600">语音录入成功</span>
                <button 
          ...
          <div className="relative">
            <textarea 
              ...
              placeholder={isWechat ? '输入或使用下方按钮...' : '手动输入，或使用下方工具识别...'}
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              <span>{isRecording ? '松开识别' : '按住说话'}</span>
            </button>
          </div>
